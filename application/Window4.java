...
public static void display(DataManager dm, int year, int month1, int day1, int month2, int day2) {
    ArrayList<String> farmNameList = dm.getFarmNames();
    totalWeight = 0;
    sort(dm,year,month1,day1,month2,day2);
    for (int i = 0; i < id.size(); i++)
      System.out.println(id.get(i));
//    for (int i = 0; i < dm.getFarmNums(); i++) 
//    	System.out.println(weight.get(i));
    System.out.println(dm.getFarmWeightForDateRange("121", 2019, 1, 1, 1, 1));

    Stage popupwindow = new Stage();

    popupwindow.initModality(Modality.APPLICATION_MODAL);
    popupwindow.setTitle("Date Range Report");
    ...
    for (int i = 0; i < farmNum; i++) {
      dataSeries1.getData().add(new XYChart.Data("Farm " + farmNameList.get(i),
          weight.get(i)));
      totalWeight=dm.getFarmWeightForDateRange(farmNameList.get(i), year, month1, day1, month2, day2)+totalWeight;
    }
    ...
    for (int i = 0; i < farmNum; i++) {
      ...
      Label l3 = new Label(
          Double.toString((double) weight.get(i) / totalWeight * 100).substring(0, 3) + "%       ");
      gridPane.add(l1, 0, i);
      gridPane.add(l2, 2, i);
      gridPane.add(l3, 4, i);
    }
   ...
   */
  private static void sort(DataManager dm, int year,int month1, int day1,int month2,int day2) {
    ArrayList<String> farmNameList = dm.getFarmNames();
    weight.clear();
    id.clear();
    for (int i = 0; i < dm.getFarmNums(); i++) {
      weight.add(dm.getFarmWeightForDateRange(farmNameList.get(i),year,month1,day1,month2,day2));
